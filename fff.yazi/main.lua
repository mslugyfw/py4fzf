...
function M:entry()
	...
	if not output then
		return ya.notify { title = "PY4FZF", content = tostring(err), timeout = 5, level = "error" }
	end
...
function M.run_with(cwd, selected)
	...
	if not child then
		return nil, Err("Failed to start `fff`, error: %s", err)
	end
	...
	if not output then
		return nil, Err("Cannot read `fff` output, error: %s", err)
	elseif not output.status.success and output.status.code ~= 130 then
		return nil, Err("`fff` exited with error code %s", output.status.code)
	end
